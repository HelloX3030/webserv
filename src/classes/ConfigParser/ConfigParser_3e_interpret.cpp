...
ListenAddress ConfigParser::parse_host_port(const Token& t)
{
    ...
    {
        addr.host = t.value.substr(0, pos);
        addr.port = parse_port(t.value.substr(pos + 1), t.line);
    }
    else
    {
        addr.port = parse_port(t.value, t.line);
    }
    return addr;
}
...
size_t ConfigParser::parse_size(const Token& t)
{
